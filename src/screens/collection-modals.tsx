...
export function CardModal({ brief, entry, onClose, onAdded, onBusyChange }: { brief: CardBrief; entry?: Entry; onClose: () => void; onAdded: (card: Card, newIds: number[], quantity: number) => void; onBusyChange: (busy: boolean) => void }) {
  ...
  return <Sheet title={entry ? 'Inside your binder' : 'Is this your card?'} onClose={onClose} busy={busy}><ScrollView contentContainerStyle={m.content} keyboardShouldPersistTaps="handled">
    ...
    {card && <>
      <View style={m.cardHero}><CardArt key={card.id} card={card} high style={{ width: '62%', maxWidth: 245 }} /><View style={m.languageTag}><Txt style={{ fontWeight: '800', fontSize: 12 }}>{card.language === 'ja' ? '日本語 · Japanese' : 'English'}</Txt></View></View>
      ...
      {liveEntry ? <><View style={ui.between}><View><Txt style={ui.subtitle}>Copies in your binder</Txt><Txt muted style={{ fontSize: 13 }}>{FINISH_LABELS[liveEntry.finish]}</Txt></View><View style={m.stepper}><IconButton icon="minus" label="Remove one copy" onPress={() => liveEntry.quantity === 1 ? setRemoving(true) : run(() => updateTrainer(t => updateQuantity(t, liveEntry.key, liveEntry.quantity - 1)))} /><Txt style={m.stepperNumber}>{liveEntry.quantity}</Txt><IconButton icon="plus" label="Add one copy" onPress={() => run(() => updateTrainer(t => updateQuantity(t, liveEntry.key, liveEntry.quantity + 1)))} /></View></View>
        {removing && <View style={m.removeBox}><Txt style={{ fontWeight: '800' }}>Remove your last copy?</Txt><Txt style={{ fontSize: 13 }}>This printing will leave your binder. You can add it again later.</Txt><View style={ui.row}><Button title="Keep it" onPress={() => setRemoving(false)} secondary style={{ flex: 1 }} /><Button title="Remove card" onPress={() => run(async () => { await updateTrainer(t => updateQuantity(t, liveEntry.key, 0)); onClose(); })} busy={busy} style={{ flex: 1 }} /></View></View>}
